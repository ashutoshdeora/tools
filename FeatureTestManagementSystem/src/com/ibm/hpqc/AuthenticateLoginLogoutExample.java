...
public class AuthenticateLoginLogoutExample {
	...
	public void authenticateLoginLogoutExample(final String serverUrl,
		...
		// if we're authenticated we'll get a null, otherwise a URL where we
		// should login at (we're not logged in, so we'll get a URL).
		...
		boolean loginResponse = example.login(authenticationPoint, username,
				password);
