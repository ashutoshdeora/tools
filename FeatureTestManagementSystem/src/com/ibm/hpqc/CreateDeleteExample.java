...
public class CreateDeleteExample {
	...
	public static void main(String[] args) throws Exception {
		new CreateDeleteExample().createDeleteExample("http://"
				+ Constants.HOST + ":" + Constants.PORT + "/qcbin",
	...
	public void createDeleteExample(final String serverUrl,
		...
		String newlyCreatedEntityUrl = example.createEntity(requirementsUrl,
				Constants.entityToPostXml);

		String deletedEntityResponse = example
				.deleteEntity(newlyCreatedEntityUrl);
		...
		Response postedEntityResponse = con.httpPost(requirementsUrl,
				Constants.entityToPostXml.getBytes(), requestHeaders);

		Assert.assertEquals("failed to post new entity.",
		...
		Assert.assertTrue(
				...
				postedEntityResponseXml
						.contains(Constants.entityToPostFieldXml));

		// getting the location of the new entity from the post response...
		newlyCreatedEntityUrl = postedEntityResponse.getResponseHeaders()
				.get("Location").iterator().next();
	...
	public String createEntity(String collectionUrl, String postedEntityXml)
		 ...
		 * header.
		 */
		String entityUrl = response.getResponseHeaders().get("Location")
				.iterator().next();
